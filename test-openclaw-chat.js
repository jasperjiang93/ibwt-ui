...
console.log('Testing OpenClaw Chat...\n');
  ...
  if (frame.type === 'res') {
    ...
    switch (frame.event) {
      ...
      case 'chat':
        ...
        console.log('\n📨 Chat event:', JSON.stringify(payload, null, 2));
        if (payload.state === 'delta' && payload.message) {
          let chunk = '';
          if (typeof payload.message === 'string') {
            chunk = payload.message;
          } else if (Array.isArray(payload.message)) {
            chunk = payload.message.map(m => typeof m === 'string' ? m : JSON.stringify(m)).join('');
          } else {
            chunk = JSON.stringify(payload.message);
          }
